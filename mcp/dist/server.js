...
const mcpUserSchema = new mongoose_1.default.Schema({
    name: String,
    email: String,
...
const McpUser = mongoose_1.default.models.User || mongoose_1.default.model('User', mcpUserSchema);
    ...
    try {
        switch (name) {
            case 'save_user_profile': {
                ...
                let user;
                try {
                    user = await McpUser.findByIdAndUpdate(userId, { name, email, favoriteSport, favoriteTeam, budgetPreference }, { new: true, upsert: true });
                }
                catch (dbErr) {
